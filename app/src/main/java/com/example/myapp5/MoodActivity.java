...
//        LineData lineData=new LineData(lineDataSet);//把第一个DataSet对象添加到Data
  //      lineData.addDataSet(lineDataSet2);      //第二个
    ...
    public void saveArray(List<Integer> list) {
        preferences = getSharedPreferences("value",Context.MODE_PRIVATE);
        SharedPreferences.Editor editor= preferences.edit();
        editor.putInt("value_size",list.size());

        for(int i=0;i<list.size();i++) {
            editor.remove("value_" + i);
            editor.putInt("value_" + i, list.get(i));
        }
        editor.apply();
    }

    public void loadArray(List<Integer> list) {

        preferences = getSharedPreferences("value",Context.MODE_PRIVATE);
        list.clear();
        int size = preferences.getInt("value_size", 0);
        for(int i=0;i<size;i++) {
            list.add(preferences.getInt("value_" + i,0));
